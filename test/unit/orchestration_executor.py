...
def test_complete_orchestration_actions():
    ...
    new_events = [helpers.new_execution_started_event(
...
def test_nondeterminism_expected_activity_call_no_task_id():
    ...
    assert "1" in complete_action.failureDetails.errorMessage  # task ID
    # expected method name
    assert "call_activity" in complete_action.failureDetails.errorMessage
...
def test_nondeterminism_wrong_activity_name():
    ...
    assert "original_activity" in complete_action.failureDetails.errorMessage
    # unexpected activity name
    assert "dummy_activity" in complete_action.failureDetails.errorMessage
...
def test_nondeterminism_expected_sub_orchestration_task_completion_no_task():
    ...
    assert "1" in complete_action.failureDetails.errorMessage  # task ID
    # expected method name
    assert "call_sub_orchestrator" in complete_action.failureDetails.errorMessage
...
def test_nondeterminism_expected_sub_orchestration_task_completion_wrong_task_type():
    ...
    def orchestrator(ctx: task.OrchestrationContext, _):
        ...
        result = yield ctx.create_timer(datetime.utcnow())
        return result
...
def test_raise_event():
    ...
    new_events = [helpers.new_event_raised_event(
        "my_event", encoded_input="42")]
    executor = worker._OrchestrationExecutor(registry, TEST_LOGGER)
    actions = executor.execute(TEST_INSTANCE_ID, old_events, new_events)
    complete_action = get_and_validate_single_complete_orchestration_action(
        actions)
...
def test_raise_event_buffered():
    ...
    new_events = [
        helpers.new_orchestrator_started_event(),
        helpers.new_execution_started_event(
            orchestrator_name, TEST_INSTANCE_ID),
    ...
    old_events = new_events + \
        [helpers.new_timer_created_event(1, timer_due_time)]
...
def test_continue_as_new(save_events: bool):
    ...
    for i in range(len(complete_action.carryoverEvents)):
        ...
        assert event.eventRaised.name.casefold() == "my_event".casefold()
